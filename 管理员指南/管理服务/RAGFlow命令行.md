...
RAGFlow CLI 是一个基于命令行的系统管理工具，为管理员提供了高效灵活的系统交互与控制方式。它基于客户端-服务器架构运行，与 Admin 服务实时通信，接收管理员命令并动态返回执行结果。
...
3. 启动 CLI 客户端：
   ...
   **参数说明：**
   ...
   - -h：RAGFlow admin 服务器的主机地址

   - -p：RAGFlow admin 服务器的端口
...
- 显示由 **邮箱**指定的用户的详细信息和权限。用户名必须用单引号或双引号括起来。
